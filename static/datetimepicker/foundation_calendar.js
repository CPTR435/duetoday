...
$.fcdp = {
	...
	buildCalendar: function(opts) {
		...
		if (dp = opts.datePicker){
			...
			var header = $('<div class="header"></div>');
			header.append('<a href="#" onclick="return false;" class="month-nav prev"><span></span</a>');
			header.append('<a href="#" onclick="return false;" class="month-nav next"><span></span></a>');
	...
	buildDayUI: function(opts, day_opts) {
		...
		if (!response) {
			...
			if (day_opts.is_clickable) {
				response = '<a href="#" onclick="return false;" class="' + clazz + '" data-date="' + day_opts.date.format() + '">' + day_num + '</a>';
			} else {
